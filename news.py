...
def display_news(list_of_news, news_quantity):
    ...
    for news in list_of_news:
        ...
        if news_data.publish_date is not None:
            ts = news_data.publish_date.timestamp()
            st.success("Published Date: " + str(datetime.fromtimestamp(ts)))
        if c >= news_quantity:
            break
def run():
